...
| Safe admin impersonation | `requireSuperAdmin()` + admin service exist; needs a distinct impersonation session (not a real `OrganizationMember` row) so it's time-boxed and clearly audited |
| Platform settings page (feature flags, plan config) | `OrganizationSetting.featureFlags` + `Plan`/`Subscription` models exist; no platform-wide settings UI yet |
